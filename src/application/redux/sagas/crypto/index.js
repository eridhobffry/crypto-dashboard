...
import { responseCryptoDataMapper, responseCryptoRawDataMapper } from "./response_mappers";
...
function* getCryptoListingSaga(action) {
    try {
        ...
        const minRawData = responseCryptoRawDataMapper(res.data)
        yield put(populateCryptoListingData({ fullData: res.data, minData, minRawData, totalCount: res.status.total_count, status: res.status}))
